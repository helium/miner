...
group_change_test(Config) ->
    ...
    ?assertEqual(
        {ok, totes_garb},
        ct_rpc:call(hd(Miners), blockchain, config, [garbage_value, Ledger1])
    ),
    ...
    ?assertEqual(
        {error, not_found},
        ct_rpc:call(hd(Miners), blockchain, config, [garbage_value, Ledger2])
    ),
...
master_key_test(Config) ->
    ...
    _ = [
        ok = ct_rpc:call(Miner, blockchain_worker, submit_txn, [Txn2_2])
     || Miner <- Miners
    ],
    ...
    _ = [
        ok = ct_rpc:call(Miner, blockchain_worker, submit_txn, [Txn4_1])
     || Miner <- Miners
    ],
    ...
    _ = [
        ok = ct_rpc:call(Miner, blockchain_worker, submit_txn, [Txn5_1])
     || Miner <- Miners
    ],
    ...
    Txn7_0 = blockchain_txn_vars_v1:new(
        #{garbage_value => goat_jokes_are_so_single_key},
        6,
    ...
    Txn7 = blockchain_txn_vars_v1:proof(Txn7_1, Proof7),
    _ = [ok = ct_rpc:call(M, blockchain_worker, submit_txn, [Txn7]) || M <- Miners],
    ok = miner_ct_utils:wait_for_chain_var_update(
        Miners,
        garbage_value,
        goat_jokes_are_so_single_key
    ),
    ...
    Txn13 = mvars(
        #{garbage_value => lets_all_hate_on_sheep},
        10,
        [Priv5, Priv6, Priv7]
    ),
...
mvars(Map, Nonce, Privs) ->
    ...
    Proofs = [
        blockchain_txn_vars_v1:create_proof(P, Txn0)
     || P <- Privs
    ],
...
version_change_test(Config) ->
    ...
    _ = [
        ok = ct_rpc:call(Miner, blockchain_worker, submit_txn, [Txn1_1])
     || Miner <- Miners
    ],
    ...
    _ = [
        ok = ct_rpc:call(Miner, blockchain_worker, submit_txn, [Txn2_1])
     || Miner <- Miners
    ],
    ...
    _ = [
        ok = ct_rpc:call(Miner, blockchain_worker, submit_txn, [Txn3_1])
     || Miner <- Miners
    ],
