...
poc_dist_v4_partitioned_test(Config) ->
    ...
    run_dist_with_params(
        poc_dist_v4_partitioned_test,
        Config,
        maps:put(?poc_version, 4, CommonPOCVars)
    ).
...
poc_dist_v5_partitioned_test(Config) ->
    ...
    run_dist_with_params(
        poc_dist_v5_partitioned_test,
        Config,
        maps:put(?poc_version, 5, CommonPOCVars)
    ).
...
poc_dist_v6_partitioned_test(Config) ->
    ...
    run_dist_with_params(
        poc_dist_v6_partitioned_test,
        Config,
        maps:put(?poc_version, 6, CommonPOCVars)
    ).
...
poc_dist_v7_partitioned_test(Config) ->
    ...
    run_dist_with_params(
        poc_dist_v7_partitioned_test,
        Config,
        maps:put(?poc_version, 7, CommonPOCVars)
    ).
...
poc_dist_v8_partitioned_test(Config) ->
    ...
    run_dist_with_params(
        poc_dist_v8_partitioned_test,
        Config,
        maps:merge(CommonPOCVars, ExtraVars)
    ).
    ...
    Res = lists:map(
        fun({_, Receipt}) ->
            ...
            PathLength > 1 andalso
                PathLength =< 4 andalso check_partitions(TestCase, Path, ActiveGateways)
        end,
        TaggedReceipts
    ),
    ct:pal("Res: ~p", [Res]),
