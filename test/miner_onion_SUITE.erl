...
basic(Config) ->
    ...
    {ok,
        {{127, 0, 0, 1}, 5678,
            <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?PULL_ACK:8/integer-unsigned>>}} = gen_udp:recv(
        Sock,
        0,
        5000
    ),
    ...
    Packet = longfi:serialize(
        ...
        longfi:new(monolithic, 0, 1, 0, Onion, #{})
    ),
    ok = gen_udp:send(
        Sock,
        "127.0.0.1",
    ...
    {ok,
        {{127, 0, 0, 1}, 5678,
            <<?PROTOCOL_2:8/integer-unsigned, Token:2/binary, ?PUSH_ACK:8/integer-unsigned>>}} = gen_udp:recv(
        Sock,
        0,
        5000
    ),
    {ok,
        {{127, 0, 0, 1}, 5678,
            <<?PROTOCOL_2:8/integer-unsigned, _Token1:2/binary, ?PULL_RESP:8/integer-unsigned,
                PacketJSON1/binary>>}} = gen_udp:recv(Sock, 0, 5000),
    ...
    {ok,
        {{127, 0, 0, 1}, 5678,
            <<?PROTOCOL_2:8/integer-unsigned, Token2:2/binary, ?PULL_RESP:8/integer-unsigned,
                PacketJSON2/binary>>}} = gen_udp:recv(Sock, 0, 5000),
    gen_udp:send(
        Sock,
        {127, 0, 0, 1},
        5678,
