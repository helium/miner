...
handle_command({verified_txn, Txn}, State=#state{chain=Chain}) ->
    lager:info("got verified transaction"),
...
handle_command({Sender, SenderRef, Txn}, #state{chain=Chain, hbbft=HBBFT}) ->
    Buf = hbbft:buf(HBBFT),
    case lists:member(blockchain_txn:serialize(Txn), Buf) of
        ...
        false ->
            ...
            spawn(fun() ->
                          Owner = self(),
                          Attempt = make_ref(),
                          Timeout = application:get_env(miner, txn_validation_budget_ms, 15000),
                          {Pid, Ref} =
                          spawn_monitor(
                            fun() ->
                                    case blockchain_txn:is_valid(Txn, Chain) of
                                        ok ->
                                            Owner ! {Attempt, ok};
                                        Error ->
                                            lager:info("hbbft_handler is_valid failed for ~p, error: ~p", [Txn, Error]),
                          ...
                          receive
                              {Attempt, ok} ->
                                  ...
                                  %% send to Owner now it's been validated
                                  Reply = libp2p_group_relcast_server:handle_command(Parent, {verified_txn, Txn}),
                                  lager:info("Transaction ~p validated ok ~p", [Txn, Reply]),
                                  Sender ! {SenderRef, Reply};
                              {Attempt, {error, Error}} ->
                                  lager:warning("hbbft_handler speculative absorb failed for ~p, error: ~p", [Txn, Error]),
                                  erlang:demonitor(SenderRef, [flush]),
                                  Sender ! {Ref, {error, Error}};
                              {'DOWN', Ref, process, _Pid, Reason} ->
                                  lager:warning("hbbft_handler speculative absorb crashed on ~p, reason: ~p", [Txn, Reason]),
                                  {reply, Reason, ignore}
                          after Timeout ->
                                    erlang:demonitor(Ref, [flush]),
                                    ...
                                    Sender ! {SenderRef, deadline}
                          end
                  end)
    end,
    {reply, pending, ignore}.
